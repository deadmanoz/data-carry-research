...
use crate::common::decoded_data_helpers::{
    stamps_file_path, stamps_subdir, stamps_txid, unwrap_counterparty, unwrap_datastorage,
    unwrap_omni, unwrap_stamps, unwrap_stamps_html, unwrap_stamps_image, unwrap_stamps_json,
};
...
async fn test_decode_bitcoin_stamps_image_54fdeda9() -> anyhow::Result<()> {
    ...
    let decoded_image = unwrap_stamps_image(result);
...
async fn test_decode_stamps_original_image() -> anyhow::Result<()> {
    ...
    println!("Testing Bitcoin Stamps original image decoding");
    println!("Transaction: {}", STAMPS_ORIGINAL_IMAGE_TXID);

    let result = decoder.decode_txid(STAMPS_ORIGINAL_IMAGE_TXID).await?;
    let stamps_data = unwrap_stamps(result);

    assert_eq!(stamps_txid(&stamps_data), STAMPS_ORIGINAL_IMAGE_TXID);
    assert!(
        stamps_file_path(&stamps_data).exists(),
        "Stamps file should exist"
    );

    let expected_dir = temp_dir
        .path()
        .join("bitcoin_stamps")
        .join(stamps_subdir(&stamps_data));
    assert!(
        stamps_file_path(&stamps_data).starts_with(&expected_dir),
        "Stamps file should be in correct directory"
    );

    println!("Stamps validation passed");
    Ok(())
}
...
async fn test_decode_stamps_classic_4d89d7() -> anyhow::Result<()> {
    ...
    println!("Testing Bitcoin Stamps classic decoding");
    println!("Transaction: {}", STAMPS_CLASSIC_4D89D7_TXID);

    let result = decoder.decode_txid(STAMPS_CLASSIC_4D89D7_TXID).await?;
    let stamps_data = unwrap_stamps(result);

    assert_eq!(stamps_txid(&stamps_data), STAMPS_CLASSIC_4D89D7_TXID);
    assert!(
        stamps_file_path(&stamps_data).exists(),
        "Stamps file should exist"
    );
...
async fn test_decode_stamps_classic_f35382() -> anyhow::Result<()> {
    ...
    println!("Testing Bitcoin Stamps classic decoding");
    println!("Transaction: {}", STAMPS_CLASSIC_F35382_TXID);

    let result = decoder.decode_txid(STAMPS_CLASSIC_F35382_TXID).await?;
    let stamps_data = unwrap_stamps(result);

    assert_eq!(stamps_txid(&stamps_data), STAMPS_CLASSIC_F35382_TXID);
    assert!(
        stamps_file_path(&stamps_data).exists(),
        "Stamps file should exist"
    );
...
async fn test_decode_stamps_recent_c8c383() -> anyhow::Result<()> {
    ...
    println!("Testing Bitcoin Stamps recent decoding");
    println!("Transaction: {}", STAMPS_RECENT_C8C383_TXID);

    let result = decoder.decode_txid(STAMPS_RECENT_C8C383_TXID).await?;
    let stamps_data = unwrap_stamps(result);

    assert_eq!(stamps_txid(&stamps_data), STAMPS_RECENT_C8C383_TXID);
    assert!(
        stamps_file_path(&stamps_data).exists(),
        "Stamps file should exist"
    );
...
async fn test_decode_stamps_image_359aef() -> anyhow::Result<()> {
    ...
    println!("Testing Bitcoin Stamps decoding");
    println!("Transaction: {}", STAMPS_IMAGE_359AEF_TXID);

    let result = decoder.decode_txid(STAMPS_IMAGE_359AEF_TXID).await?;
    let stamps_data = unwrap_stamps(result);

    assert_eq!(stamps_txid(&stamps_data), STAMPS_IMAGE_359AEF_TXID);
    assert!(
        stamps_file_path(&stamps_data).exists(),
        "Stamps file should exist"
    );
...
async fn test_decode_stamps_image_50aeb7() -> anyhow::Result<()> {
    ...
    println!("Testing Bitcoin Stamps decoding");
    println!("Transaction: {}", STAMPS_IMAGE_50AEB7_TXID);

    let result = decoder.decode_txid(STAMPS_IMAGE_50AEB7_TXID).await?;
    let stamps_data = unwrap_stamps(result);

    assert_eq!(stamps_txid(&stamps_data), STAMPS_IMAGE_50AEB7_TXID);
    assert!(
        stamps_file_path(&stamps_data).exists(),
        "Stamps file should exist"
    );
...
async fn test_decode_stamps_image_582a46() -> anyhow::Result<()> {
    ...
    println!("Testing Bitcoin Stamps decoding");
    println!("Transaction: {}", STAMPS_IMAGE_582A46_TXID);

    let result = decoder.decode_txid(STAMPS_IMAGE_582A46_TXID).await?;
    let stamps_data = unwrap_stamps(result);

    assert_eq!(stamps_txid(&stamps_data), STAMPS_IMAGE_582A46_TXID);
    assert!(
        stamps_file_path(&stamps_data).exists(),
        "Stamps file should exist"
    );
...
async fn test_decode_stamps_image_5e7d66() -> anyhow::Result<()> {
    ...
    println!("Testing Bitcoin Stamps decoding");
    println!("Transaction: {}", STAMPS_IMAGE_5E7D66_TXID);

    let result = decoder.decode_txid(STAMPS_IMAGE_5E7D66_TXID).await?;
    let stamps_data = unwrap_stamps(result);

    assert_eq!(stamps_txid(&stamps_data), STAMPS_IMAGE_5E7D66_TXID);
    assert!(
        stamps_file_path(&stamps_data).exists(),
        "Stamps file should exist"
    );
...
async fn test_decode_stamps_counterparty_transport_31a96a3b() -> anyhow::Result<()> {
    ...
    println!("Counterparty transport validation passed - STAMP: signature correctly detected");
    println!("Decoded image matches reference perfectly!");
    Ok(())
}

// =============================================================================
// BITCOIN STAMPS TESTS - JSON (SRC-20, SRC-101)
// =============================================================================
...
async fn test_decode_bitcoin_stamps_src101_77fb147b() -> anyhow::Result<()> {
    ...
    let decoded_json = unwrap_stamps_json(result);
...
async fn test_decode_bitcoin_stamps_multi_output_95dca4() -> anyhow::Result<()> {
    ...
    println!("Testing Bitcoin Stamps HTML content decoding (multi-output, Counterparty-embedded)");
    println!("Transaction: {}", STAMPS_MULTI_OUTPUT_TXID);

    let result = decoder.decode_txid(STAMPS_MULTI_OUTPUT_TXID).await?;
    let decoded_html = unwrap_stamps_html(result);
...
async fn test_decode_counterparty_issuance_e5e9f6a6() -> anyhow::Result<()> {
    ...
    let data = unwrap_counterparty(result);
...
async fn test_decode_counterparty_type0_585f50f1() -> anyhow::Result<()> {
    ...
    println!("Testing Counterparty Type 0 decoding");
...
async fn test_decode_counterparty_type30_627ae48d() -> anyhow::Result<()> {
    ...
    println!("Testing Counterparty Type 30 decoding");
...
async fn test_decode_counterparty_subasset_793566ef() -> anyhow::Result<()> {
    ...
    println!("Testing Counterparty subasset decoding");
    println!("Transaction: {}", COUNTERPARTY_SUBASSET_TXID);

    let result = decoder.decode_txid(COUNTERPARTY_SUBASSET_TXID).await?;
    let data = unwrap_counterparty(result);

    println!("Successfully decoded Counterparty subasset");
    assert_eq!(data.txid, COUNTERPARTY_SUBASSET_TXID);
...
async fn test_decode_counterparty_modern_1of3_a63ee2b1() -> anyhow::Result<()> {
    ...
    let _data = unwrap_counterparty(result);
...
async fn test_decode_omni_send_to_owners_0937f1() -> anyhow::Result<()> {
    ...
    println!("Testing Omni Layer Send-to-Owners decoding");
...
async fn test_decode_omni_dex_offer_9a0177() -> anyhow::Result<()> {
    ...
    println!("Testing Omni Layer DEX offer decoding");
...
async fn test_decode_omni_grant_tokens_1caf04() -> anyhow::Result<()> {
    ...
    println!("Testing Omni Layer grant tokens");
...
async fn test_decode_omni_revoke_tokens_742973() -> anyhow::Result<()> {
    ...
    println!("Testing Omni Layer revoke tokens");
...
async fn test_decode_omni_dex_accept_3d7742() -> anyhow::Result<()> {
    ...
    println!("Testing Omni Layer DEX accept");
...
/// Test decoding of the Bitcoin Whitepaper embedded PDF
///
...
async fn test_decode_datastorage_bitcoin_whitepaper_pdf() -> anyhow::Result<()> {
    ...
    let result = decoder
        ...
        .await?;
    let data = unwrap_datastorage(result);
