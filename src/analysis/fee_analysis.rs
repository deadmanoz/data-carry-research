...
/// Fee analysis engine for transaction cost calculations
pub struct FeeAnalysisEngine;

impl FeeAnalysisEngine {
    ...
    pub fn analyse_transaction_fees(db: &Database) -> AppResult<FeeAnalysisReport> {
        ...
        let regular_transactions =
            conn.count_rows("enriched_transactions", Some("is_coinbase = 0"))?;

        // Get fee statistics
        let fee_statistics = Self::get_fee_statistics(db)?;

        // Get storage cost analysis
        let storage_cost_analysis = Self::get_storage_costs(db)?;
