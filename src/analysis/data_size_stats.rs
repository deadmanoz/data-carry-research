...
use crate::database::NO_MIME_TYPE_SENTINEL;
...
pub fn analyse_comprehensive_data_sizes(db: &Database) -> AppResult<ComprehensiveDataSizeReport> {
    let protocol_breakdown = analyse_protocol_data_sizes(db)?;
    let spendability_breakdown = analyse_spendability_data_sizes(db)?;
    let content_type_breakdown = analyse_content_type_spendability(db)?;
