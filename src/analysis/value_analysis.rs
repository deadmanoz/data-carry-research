...
pub fn analyse_value_distributions(db: &Database) -> AppResult<ValueDistributionReport> {
    ...
    let global_distribution = calculate_global_distribution(conn, &bucket_ranges)?;
    ...
    for protocol in protocols {
        let distribution = calculate_protocol_distribution(conn, &protocol, &bucket_ranges)?;
...
fn calculate_global_distribution(
    ...
    let percentiles = calculate_percentiles(conn, None)?;

    // Calculate bucket distributions
    let buckets = calculate_buckets(conn, bucket_ranges, None, total_outputs, total_value_sats)?;
...
fn calculate_protocol_distribution(
    ...
    let percentiles = calculate_percentiles(conn, Some(protocol))?;

    // Calculate bucket distributions
    let buckets = calculate_buckets(
        conn,
        bucket_ranges,
        Some(protocol),
        total_outputs,
...
mod tests {
    ...
    fn test_value_analysis_empty_database() {
        ...
        let result = analyse_value_distribution(&db, fee_report);
        assert!(result.is_ok());
    ...
    fn test_value_distributions_empty_database() {
        ...
        let result = analyse_value_distributions(&db);
        assert!(result.is_ok());
