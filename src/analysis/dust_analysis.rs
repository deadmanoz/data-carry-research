...
pub fn analyse_dust_thresholds(db: &Database) -> AppResult<DustAnalysisReport> {
    ...
    let classified_outputs_total: usize = protocol_breakdown.iter().map(|p| p.total_outputs).sum();
...
fn validate_consistency(report: &DustAnalysisReport) -> AppResult<()> {
    ...
    let protocol_plus_unclassified = report.classified_outputs_total + report.unclassified_count;
