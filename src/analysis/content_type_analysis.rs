...
pub fn analyse_content_types(db: &Database) -> AppResult<ContentTypeAnalysisReport> {
    ...
    let content_type_breakdown = get_content_type_breakdown(db)?;

    // Get MIME category grouping
    let category_breakdown = get_category_breakdown(db)?;

    // Get protocol-specific content type stats
    let protocol_breakdown = get_protocol_content_types(db)?;

    // Get valid None stats (architecturally correct)
    let valid_none_stats = get_valid_none_stats(db)?;

    // Get invalid None stats (missing content types)
    let invalid_none_stats = get_invalid_none_stats(db)?;
...
fn get_valid_none_stats(db: &Database) -> AppResult<ValidNoneStats> {
    ...
    let total_valid_none = likely_data_storage + likely_legitimate + stamps_unknown + omni_failed;
