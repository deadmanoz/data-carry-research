        ...
        if let Some(obj) = parsed.as_object() {
            ...
            if let Some(protocol) = obj.get("p").and_then(|v| v.as_str()) {
                return match protocol.to_lowercase().as_str() {
                    "src-20" => JsonType::SRC20,
                    "src-101" => JsonType::SRC101,
                    "src-721" => JsonType::SRC721,
                    "src-721r" => JsonType::SRC721r,
                    _ => JsonType::Generic,
                };
            }
        }
...
pub mod validation {
    ...
    pub fn detect_stamps_variant_with_content(
        ...
        // 1. EXTRACT RAW DATA FIRST (after stamp signature)
        let raw_data = if let Some((sig_offset, sig_variant)) = find_stamp_signature(decrypted_data)
        {
            // Use actual signature length, not hardcoded 6
            let sig_len = sig_variant.len();
            let data_start = sig_offset + sig_len;

            // Safety check for truncated payloads (signature-only stamps)
            if decrypted_data.len() > data_start {
                &decrypted_data[data_start..]
            } else {
                // Signature present but no data after it
                return (Some(StampsVariant::Unknown), None, None);
            }
        } else {
            // No stamp signature found - shouldn't happen but handle gracefully
            decrypted_data
        };

        // Try base64 decoding - many Bitcoin Stamps use base64 encoding
        let payload_vec;
        let payload = if let Some(base64_bytes) = extract_base64_after_stamp(decrypted_data) {
            if let Ok(decoded) = BASE64_LENIENT.decode(&base64_bytes) {
                payload_vec = decoded;
    ...
    ///
    /// This handles both pure Bitcoin Stamps and Counterparty-embedded Bitcoin Stamps:
    /// 1. Pure Bitcoin Stamps: length prefix + stamp signature + data
    /// 2. Counterparty-embedded: "CNTRPRTY" + other data + stamp signature + data
    ///
    /// Detects all signature variants (stamp:/STAMP:/stamps:/STAMPS:) case-insensitively.
    pub fn extract_base64_after_stamp(decrypted_data: &[u8]) -> Option<Vec<u8>> {
        if decrypted_data.len() < 6 {
            return None; // Too small to contain minimum signature length
        }

        // Try pure Bitcoin Stamps format first (length-prefixed)
        if decrypted_data.len() >= 8 {
            let chunk_length = ((decrypted_data[0] as usize) << 8) | (decrypted_data[1] as usize);

            // Only try length-based extraction if the length seems reasonable
            if chunk_length > 0 && chunk_length <= decrypted_data.len() - 2 {
                let chunk = &decrypted_data[2..2 + chunk_length];

                // Check for any stamp signature variant in the length-prefixed chunk
                if let Some((sig_pos, sig_variant)) = find_stamp_signature(chunk) {
                    if sig_pos == 0 {
                        // Signature at start of chunk (expected for length-prefixed format)
                        let after_prefix = &chunk[sig_variant.len()..];
                        if !after_prefix.is_empty() {
                            return Some(after_prefix.to_vec());
                        }
                    }
                }
            }
        ...
        // Handle Bitcoin Stamps with Counterparty transport: check for stamp signature at any position
        // The 2 bytes before signature might be length information
        if let Some((pos, sig_variant)) = find_stamp_signature(decrypted_data) {
            // For Counterparty-embedded stamps, parse following Electrum-Counterparty reference
            let after_stamp = &decrypted_data[pos + sig_variant.len()..];
            if !after_stamp.is_empty() {
                // Convert to string for parsing (handling potential non-UTF8 bytes gracefully)
                let data_str = String::from_utf8_lossy(after_stamp);

                // Parse Bitcoin Stamps formats following Electrum-Counterparty logic:
                // 1. stamp:base64data
                // 2. stamp:image/png;base64data
                // 3. stamp:data:image/png;base64,base64data
                let mut image_data = data_str.as_ref();

                // Remove MIME type if present (e.g., "image/png;base64data" -> "base64data")
                if let Some(last_semi) = image_data.rfind(';') {
                    let after_last_semi = &image_data[last_semi + 1..];
                    ...
                    {
                        image_data = after_last_semi;
                    }
                }

                // Remove data URI prefix if present (e.g., "data:image/png;base64,data" -> "data")
                if let Some(comma_pos) = image_data.rfind(',') {
                    let after_comma = &image_data[comma_pos + 1..];
                    ...
                    if after_comma.len() > 10
                        && after_comma
                            ...
                            .all(|c| c.is_ascii_alphanumeric() || c == '+' || c == '/' || c == '=')
                    {
                        image_data = after_comma;
                    }
                }

                // Trim whitespace and return as owned bytes
